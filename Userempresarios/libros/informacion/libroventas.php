...
//--------------------------FACTURAS
$tot = $mysqli->query("SELECT sum(TotalFact),sum(TotalIva),sum(TotalNeto),sum(TotalExento),sum(TotalEspyRet) FROM facturaventa where Mestrab='$mestrabajo'  AND anoTrab='$anotrabajo' and rutEmpre='$Rutempre' ");
...
$espe=$tota[4];
//--------------------------boletas
$bole = $mysqli->query("SELECT sum(total),sum(totaliva),sum(neto) FROM boleta where Mestrab='$mestrabajo'  AND anoTrab='$anotrabajo' and rutEmpre='$Rutempre' ");
...
$fac = $mysqli->query("SELECT numeroFactVenta, TotalFact, TotalNeto, TotalIva, TotalEspyRet,TotalExento ,TipoDoc FROM facturaventa WHERE Mestrab='$mestrabajo'  AND anoTrab='$anotrabajo' and rutEmpre='$Rutempre' ");

$bol = $mysqli->query("SELECT  desde, hasta, neto, totaliva, total,tipoDoc FROM boleta WHERE Mestrab='$mestrabajo'  AND anoTrab='$anotrabajo' and rutEmpre='$Rutempre'");
...
<center>
<p style="padding: 2px 0px 2px 0px; font-size: 20px" class="bg-primary">Resumen de venta de la empresa <?php echo $nombreEmpresa ?> - <?php echo $Rutempre ?></</p></center>
...
<div class="container">
  ...
  <a href="../../opciones.php?rut=<?php echo$Rutempre ?>" class="btn btn-primary">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Volver a opciones&nbsp;&nbsp;&nbsp;&nbsp;</a>
  </div>
...
if($row[6]=='46'){
  ...
  window.location='opciones.php?rut='+rutt;
		...
		//-------------------------MOSTRAR BOLETAS-----------------------------------------------------
		   while($row = $bol ->fetch_array()){
if($row[5]=='2'){
	$tipoo='Boleta manual';
}
if($row[5]=='3'){
	$tipoo='Boleta electrónica';
}
