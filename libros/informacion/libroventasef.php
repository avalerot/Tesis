   ...
   if(isset($_POST['mes1']) AND isset($_POST['mes2']) AND isset($_POST['anotrabajo1'])){
	   ...
	   $anotrabajo1=$_POST['anotrabajo1'];
		...
		if($consulta!=""){ $tot = $mysqli->query("SELECT sum(TotalFact),sum(TotalIva),sum(TotalNeto),sum(TotalExento),sum(TotalEspyRet) FROM facturaventa where rutEmpre='$rut' AND $consulta ");
					...
					$exeen=$tota[3];
					$espe=$tota[4];
          $bole = $mysqli->query("SELECT sum(total),sum(totaliva),sum(neto) FROM boleta where rutEmpre='$rut' AND $consulta  ");
             $bolet = mysqli_fetch_array($bole, MYSQLI_NUM);
					...
					$exeen=$tota[3];
					$espe=$tota[4];
          $bole = $mysqli->query("SELECT sum(total),sum(totaliva),sum(neto) FROM boleta where rutEmpre='$rut'");
          $bolet = mysqli_fetch_array($bole, MYSQLI_NUM);
					$totalbols=$bolet[0];
					$ivabols=$bolet[1];
					$netobols=$bolet[2];
    ...
    <body>
    	...
    	<?php
        	echo" <tr style='vertical-align:middle'>
					<td style='vertical-align:middle; '>$todototal</td>
					<td style='vertical-align:middle'>$todoiva</td>
					<td style='vertical-align:middle'>$todoneto</td>
				</tr>
			";
		?>
    </tbody>
