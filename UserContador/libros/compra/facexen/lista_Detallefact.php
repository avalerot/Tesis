...
<script type="text/javascript">
	...
	function modal(rut,cd,anotrabajo,fecha,mestrabajo,numero,aux,Rutempre,cc,razon,totalespe,exen,div,url){
$.post(
	url,
{rut:rut,cd:cd,anotrabajo:anotrabajo,fecha:fecha,mestrabajo:mestrabajo,numero:numero,aux:aux,Rutempre:Rutempre,cc:cc,razon:razon,totalespe:totalespe,exen:exen},
function(resp){
        ...
        <tr href="javascript:void(0);"  onclick="modalespe('<?php echo$esbebidas ?>','<?php echo$espdisel ?>','<?php echo$espiscos ?>','<?php echo$esvinos ?>','<?php echo$retharina ?>','<?php echo$retcarne ?>','myModal','detalleEspe.php');"  data-toggle="modal" data-target="#myModal"  >
			<td style='vertical-align:middle'><?php echo $abono?></td>
					...
					<td style='vertical-align:middle'> <center><form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" >
						...
						<input type="hidden" value="<?php echo $exen ?>" name="exen">
...
<a class="btn btn-success" href="javascript:void(0);" onclick="modal('<?php echo$rut ?>','<?php echo$cd ?>','<?php echo$anotrabajo ?>','<?php echo$fecha ?>','<?php echo$mestrabajo ?>','<?php echo$numero ?>','<?php echo$aux ?>','<?php echo$Rutempre ?>','<?php echo$cc ?>','<?php echo$Razon ?>','<?php echo$totalespe ?>','<?php echo$exen ?>','myModal','Form_DetalleFctrs.php');" id="agregar_licencia" data-toggle="modal" data-target="#myModal">Agregar secuencia </a>
